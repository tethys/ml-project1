...
The degree of the polynomial was varied from 1 to 10 for both blob-modelst. In $Fig$\ref{fig:degree_blob1},\ref{fig:degre_blob2} we plotted the mean training and validation rmse error only the degree varied between 2 - 10 and 2 - 6 respectively for readability (for other values the error was too big).

Using 5-fold cross validation with $\lambda = 1e-5, \alpha = 0.1$ we
notice for the first blob, RMSE train and validation is smaller for  a polynomial of degree 3. For the second blob, using  $\lambda = 0.001, \alpha = 0.1$ we obtain a best fit for a polynomial of degree 2.
...
Note also that our final RMSE (reported on the whole training data) is smaller that the sum of the two RMSE.
