...
notice that a polynomial of degree 3 is the best fit for the first model. Increasing it further leads to overfitting, since the training errors becomes very small and the validation error increases. For the second model, using  $\lambda = 0.001, \alpha = 0.1$ we obtain a best fit for a polynomial of degree 2.
...
We note that our assumption that in our training set we have a clear separation between the two blobs using the value of feature 36 is not a realistic approximation. The problem is that there are some samples with feature 36 $>$ -13 whose predicted value should be estimated using model 1 and some which should use model 2. This can also be seen in $Fig.$\ref{fig:feature36}.
...
To test this, we tried splitting $Xtrain$ into $20\%$ for testing and $80\%$ for training and validation. We applied the same data processing, we estimated model parameters as above and the RMSE per model proved to be very similar with the ones in Table\ref{table:feat_transform}. 

However, as we expected, our test error on the held out $20\%$ got much worse. The total RMSE for our $20\%$  test data was 295.78 (RMSE= 73.5 for model 1, RMSE = 887.24 for model2 and 295.78 RMSE combined).
If we remove all the samples in the held out test data which have feature36 in the interval [-12,-14], we obtain a total RMSE = 68.62 (RMSE = 71.6 for model 1, RMSE = 82.77 for model 2) which validates our hypothesis that the bigger error comes from that part of the model. To sum up, we have two models that work well separately but perform bad for samples with feature 36 near the threshold value.

Our model is a piecewiese polynomial and can suffer from various problems (( see cite book here) for details on how to prevent this). Since we do not enforce continuity at the 36th feature, our model will generate high errors for samples that have the value of feature36 near the threshold value, in our case -13. This happens because feature 36 does not give a clear separation of the data into two distinct models. Enforcing smoothness at the thresholded value could have solved the problem, but we did not thave the time to try this.
